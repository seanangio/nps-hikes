...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('logs/trail_matcher.log'),
...
class TrailMatcher:
    ...
    def preprocess_name(self, name: str) -> str:
        ...
        words_to_remove = ['trail', 'trailhead', 'trails', 'path', 'paths', 'walk', 'walks']
    ...
    def calculate_confidence_score(self, name_similarity: float, distance_m: float) -> float:
        """
        ...
        # Combined score
        confidence = (name_similarity * self.name_weight) + (distance_score * self.distance_weight)
            ...
            if best_match['confidence_score'] >= self.confidence_threshold:
                ...
                return {
                    **gmaps_point,
                    ...
                    'min_point_to_trail_distance_m': best_match['min_point_to_trail_distance_m'],
        ...
        try:
            ...
            for _, point in gmaps_points.iterrows():
                ...
                if len(matched_data) % 20 == 0:
                    logger.info(f"Processed {len(matched_data)}/{len(gmaps_points)} points...")
            
            # Calculate final stats
            self.stats['avg_confidence_score'] = sum(confidence_scores) / len(confidence_scores) if confidence_scores else 0
            self.stats['avg_distance_m'] = sum(distances) / len(distances) if distances else 0
    ...
    def _print_summary(self) -> None:
        ...
        logger.info(f"Match rate: {((self.stats['matched_tnm'] + self.stats['matched_osm']) / self.stats['total_gmaps_points'] * 100):.1f}%")
        logger.info(f"Average confidence score: {self.stats['avg_confidence_score']:.3f}")
