...
def mock_collector():
    ...
    with patch('tnm_hikes_collector.get_postgres_engine'), \
         patch('tnm_hikes_collector.DatabaseWriter'):
...
class TestTNMHikesCollector:
    ...
    def test_clip_trails_to_boundary(self, mock_collector, sample_tnm_response, sample_park_boundary):
        """Test clipping trails to park boundary."""
        trails_gdf = mock_collector.load_trails_to_geodataframe(sample_tnm_response, "acad")
        clipped_gdf = mock_collector.clip_trails_to_boundary(trails_gdf, sample_park_boundary, "acad")
    ...
    def test_aggregate_trails_by_name(self, mock_collector, sample_tnm_response):
        ...
        gdf = gpd.GeoDataFrame(pd.concat([gdf, duplicate_gdf], ignore_index=True), crs=gdf.crs)
    ...
    def test_filter_by_minimum_length(self, mock_collector, sample_tnm_response):
        ...
        # Concatenate the two GeoDataFrames
        gdf = gpd.GeoDataFrame(pd.concat([gdf, short_gdf], ignore_index=True), crs=gdf.crs)
    ...
    def test_process_trails_full_pipeline(self, mock_collector, sample_park_boundary):
        ...
        with patch.object(mock_collector, 'query_tnm_api') as mock_query, \
             ...
             patch.object(mock_collector, 'filter_by_minimum_length') as mock_length_filter, \
             patch.object(mock_collector, 'add_metadata') as mock_metadata:
            
            # Mock responses
            mock_query.return_value = {"features": [{"type": "Feature", "geometry": {"type": "LineString", "coordinates": [[0, 0], [1, 1]]}, "properties": {"name": "Test"}}]}
...
class TestDataValidation:
    ...
    def test_trail_length_validation(self, mock_collector):
        ...
        data = {
            ...
            'name': ['Trail'] * 4,
            'lengthmiles': [0.005, 0.01, 50.0, 50.1],  # Below min, at min, at max, above max
