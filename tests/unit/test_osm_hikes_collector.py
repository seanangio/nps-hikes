...
def sample_trails_gdf():
    ...
    data = {
        ...
        'geometry_type': ['LineString', 'LineString', 'LineString', 'LineString', 'LineString'],
...
class TestOSMHikesCollector:
    ...
    def test_validate_trails_removes_invalid_geometries(self, mock_collector, sample_invalid_trails_gdf):
        ...
        assert all(geom.geom_type in ['LineString', 'MultiLineString'] for geom in result.geometry)
    
    def test_validate_trails_removes_unrealistic_lengths(self, mock_collector, sample_invalid_trails_gdf):
    ...
    def test_validate_trails_removes_duplicates(self, mock_collector, sample_invalid_trails_gdf):
...
class TestDataValidation:
    ...
    def test_trail_length_validation_boundaries(self, mock_collector):
        ...
        data = {
            ...
            'name': ['Trail'] * 4,
            'source': [None] * 4,
            'length_mi': [0.009, 0.01, 50.0, 50.1],  # Below min, at min, at max, above max
...
class TestIntegration:
    ...
    def test_process_trails_full_workflow(self, mock_engine, mock_osm_query):
        ...
        mock_trail_data = {
            ...
            'name': ['Test Trail'],
            'source': ['GPS'],
            'geometry': [LineString([(0, 0), (0.1, 0.1)])]  # About 0.01 miles, minimum valid length
        }
        ...
        assert result.iloc[0]['length_mi'] >= 0.01  # Should meet minimum length requirement
