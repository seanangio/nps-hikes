...
class TNMHikesProfiler:
    ...
    def get_trail_statistics(self, park_codes: Optional[List[str]] = None) -> Dict[str, Any]:
        """
    ...
    def get_trail_type_breakdown(self, park_codes: Optional[List[str]] = None) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            breakdown = {
                ...
                'summary': {
                    ...
                    'most_common_type': type_df.iloc[0]['trailtype'] if len(type_df) > 0 else None,
                    'most_common_designation': designation_df.iloc[0]['nationaltraildesignation'] if len(designation_df) > 0 else None
                }
            }
            
            self.logger.info(f"Generated type breakdown: {len(type_df)} types, {len(designation_df)} designations")
            return breakdown
    ...
    def get_data_quality_metrics(self, park_codes: Optional[List[str]] = None) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            df['name_completeness'] = (df['named_trails'] / df['total_trails'] * 100).round(2)
            df['length_completeness'] = (df['trails_with_length'] / df['total_trails'] * 100).round(2)
            df['type_completeness'] = (df['trails_with_type'] / df['total_trails'] * 100).round(2)
            df['hiker_status_completeness'] = (df['trails_with_hiker_status'] / df['total_trails'] * 100).round(2)
            df['maintainer_completeness'] = (df['trails_with_maintainer'] / df['total_trails'] * 100).round(2)
            df['designation_completeness'] = (df['trails_with_designation'] / df['total_trails'] * 100).round(2)
            df['geometry_completeness'] = (df['trails_with_geometry'] / df['total_trails'] * 100).round(2)
            
            # Overall quality metrics
            total_trails = df['total_trails'].sum()
            overall_quality = {
                'name_completeness': (df['named_trails'].sum() / total_trails * 100).round(2),
                'length_completeness': (df['trails_with_length'].sum() / total_trails * 100).round(2),
                'type_completeness': (df['trails_with_type'].sum() / total_trails * 100).round(2),
                'hiker_status_completeness': (df['trails_with_hiker_status'].sum() / total_trails * 100).round(2),
                'maintainer_completeness': (df['trails_with_maintainer'].sum() / total_trails * 100).round(2),
                'designation_completeness': (df['trails_with_designation'].sum() / total_trails * 100).round(2),
                'geometry_completeness': (df['trails_with_geometry'].sum() / total_trails * 100).round(2)
            }
    ...
    def get_spatial_analysis(self, park_codes: Optional[List[str]] = None) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            def parse_centroid(centroid_text):
                if centroid_text and centroid_text.startswith('POINT('):
                    coords = centroid_text.replace('POINT(', '').replace(')', '').split()
    ...
    def compare_with_osm_data(self, park_codes: Optional[List[str]] = None) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            df['trail_count_ratio'] = (df['tnm_trail_count'] / df['osm_trail_count']).round(3)
            df['length_ratio'] = (df['tnm_total_length'] / df['osm_total_length']).round(3)
            df['avg_length_ratio'] = (df['tnm_avg_length'] / df['osm_avg_length']).round(3)
            
            # Handle division by zero
            df['trail_count_ratio'] = df['trail_count_ratio'].replace([float('inf'), -float('inf')], None)
            df['length_ratio'] = df['length_ratio'].replace([float('inf'), -float('inf')], None)
            df['avg_length_ratio'] = df['avg_length_ratio'].replace([float('inf'), -float('inf')], None)
            ...
            comparison = {
                ...
                'summary': {
                    ...
                    'parks_with_both': len(df[(df['tnm_trail_count'] > 0) & (df['osm_trail_count'] > 0)]),
    ...
    def export_analysis_results(self, output_dir: str = "profiling_results", 
                               park_codes: Optional[List[str]] = None) -> Dict[str, str]:
        """
        ...
        try:
            ...
            if stats:
                stats_file = os.path.join(output_dir, f"tnm_trail_statistics_{timestamp}.json")
                pd.DataFrame(stats['park_statistics']).to_json(stats_file, orient='records', indent=2)
            ...
            if type_breakdown:
                type_file = os.path.join(output_dir, f"tnm_trail_types_{timestamp}.json")
                pd.DataFrame(type_breakdown['trail_types']).to_json(type_file, orient='records', indent=2)
                exported_files['trail_types'] = type_file
            
            # Export data quality metrics
            quality = self.get_data_quality_metrics(park_codes)
            if quality:
                quality_file = os.path.join(output_dir, f"tnm_data_quality_{timestamp}.json")
                pd.DataFrame(quality['park_quality']).to_json(quality_file, orient='records', indent=2)
                exported_files['data_quality'] = quality_file
            
            # Export spatial analysis
            spatial = self.get_spatial_analysis(park_codes)
            if spatial:
                spatial_file = os.path.join(output_dir, f"tnm_spatial_analysis_{timestamp}.json")
                pd.DataFrame(spatial['park_spatial']).to_json(spatial_file, orient='records', indent=2)
                exported_files['spatial_analysis'] = spatial_file
            
            # Export comparison with OSM
            comparison = self.compare_with_osm_data(park_codes)
            if comparison:
                comparison_file = os.path.join(output_dir, f"tnm_osm_comparison_{timestamp}.json")
                pd.DataFrame(comparison['park_comparison']).to_json(comparison_file, orient='records', indent=2)
            ...
            summary_file = os.path.join(output_dir, f"tnm_analysis_summary_{timestamp}.json")
    ...
    def generate_comprehensive_report(self, park_codes: Optional[List[str]] = None) -> Dict[str, Any]:
        """
...
def main():
    ...
    parser.add_argument("--parks", type=str, help="Comma-separated list of park codes to analyze")
    parser.add_argument("--output-dir", default="profiling_results", help="Output directory for results")
