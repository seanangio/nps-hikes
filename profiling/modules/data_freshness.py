...
class DataFreshnessMonitor:
    ...
    def get_freshness_summary(self) -> Dict[str, Dict]:
        ...
        summary["parks"] = {
            ...
            "oldest_data": (
                parks_df["collected_at"].min() if not parks_df.empty else None
            ),
            "newest_data": (
                parks_df["collected_at"].max() if not parks_df.empty else None
            ),
        }
        ...
        summary["boundaries"] = {
            ...
            "fresh_count": len(
                boundaries_df[boundaries_df["staleness_status"] == "Fresh"]
            ),
            "warning_count": len(
                boundaries_df[boundaries_df["staleness_status"] == "Warning"]
            ),
            "stale_count": len(
                boundaries_df[boundaries_df["staleness_status"] == "Stale"]
            ),
            "oldest_data": (
                boundaries_df["collected_at"].min() if not boundaries_df.empty else None
            ),
            "newest_data": (
                boundaries_df["collected_at"].max() if not boundaries_df.empty else None
            ),
        }
        ...
        summary["osm_hikes"] = {
            ...
            "fresh_count": (
                len(osm_df[osm_df["staleness_status"] == "Fresh"])
                if not osm_df.empty
                else 0
            ),
            "warning_count": (
                len(osm_df[osm_df["staleness_status"] == "Warning"])
                if not osm_df.empty
                else 0
            ),
            "stale_count": (
                len(osm_df[osm_df["staleness_status"] == "Stale"])
                if not osm_df.empty
                else 0
            ),
            "total_trails": osm_df["total_trails"].sum() if not osm_df.empty else 0,
            "oldest_data": (
                osm_df["latest_trail_collected"].min() if not osm_df.empty else None
            ),
            "newest_data": (
                osm_df["latest_trail_collected"].max() if not osm_df.empty else None
            ),
        }
        ...
        summary["tnm_hikes"] = {
            ...
            "fresh_count": (
                len(tnm_df[tnm_df["staleness_status"] == "Fresh"])
                if not tnm_df.empty
                else 0
            ),
            "warning_count": (
                len(tnm_df[tnm_df["staleness_status"] == "Warning"])
                if not tnm_df.empty
                else 0
            ),
            "stale_count": (
                len(tnm_df[tnm_df["staleness_status"] == "Stale"])
                if not tnm_df.empty
                else 0
            ),
            "total_trails": tnm_df["total_trails"].sum() if not tnm_df.empty else 0,
            "oldest_data": (
                tnm_df["latest_trail_collected"].min() if not tnm_df.empty else None
            ),
            "newest_data": (
                tnm_df["latest_trail_collected"].max() if not tnm_df.empty else None
            ),
        }
