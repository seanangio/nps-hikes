...
class VisualizationProfiler:
    ...
    def run_park_map(self):
        ...
        try:
            ...
            boundaries_gdf = gpd.read_postgis(
                boundaries_query, engine, geom_col="geometry"
            )
    ...
    def _create_park_map(self, parks_df, boundaries_gdf):
        ...
        if not parks_df.empty:
            ...
            points_gdf = gpd.GeoDataFrame(
                ...
                geometry=[
                    Point(lon, lat)
                    for lon, lat in zip(parks_df["longitude"], parks_df["latitude"])
                ],
                crs="EPSG:4326",
            )
    ...
    def _add_park_labels(self, ax, parks_df):
        ...
        for idx, row in parks_df.iterrows():
            ...
            ax.plot(
                [lon, label_x], [lat, label_y], color="black", linewidth=0.5, alpha=0.7
            )
    ...
    def run_all(self):
        ...
        # For modules without SQL queries, run specific methods directly
        if not self.config["queries"]:
            # Run the main visualization method
            self.run_park_map()
        else:
            # Run methods based on SQL query files (legacy approach)
