...
SELECT 
    ...
    CASE 
        ...
        THEN ROUND(COUNT(DISTINCT t.permanentidentifier)::numeric / COUNT(DISTINCT o.osm_id), 3)
        ELSE NULL 
    END as trail_count_ratio,
    CASE 
        WHEN COALESCE(SUM(o.length_mi), 0) > 0 
        THEN ROUND((COALESCE(SUM(t.lengthmiles), 0)::numeric / COALESCE(SUM(o.length_mi), 0)::numeric), 3)
        ELSE NULL 
    END as length_ratio,
    CASE 
        WHEN COALESCE(AVG(o.length_mi), 0) > 0 
        THEN ROUND((COALESCE(AVG(t.lengthmiles), 0)::numeric / COALESCE(AVG(o.length_mi), 0)::numeric), 3)
        ELSE NULL 
