...
SELECT
    ...
    ROUND(
        (COUNT(CASE WHEN primary_trail_maintainer IS NOT NULL THEN 1 END)::numeric / COUNT(*) * 100), 2
    ) as maintainer_completeness_percent,
    ROUND(
        (COUNT(CASE WHEN national_trail_designation IS NOT NULL THEN 1 END)::numeric / COUNT(*) * 100), 2
